...
@section('content')
    ...
    <style id="qb-skin-overrides">
        ...
        .filter-button-item .filter-label {
            font-size: 11px;
            color: transparent;
            margin-bottom: 4px;
            height: 15px
        ...
        .filter-button-item .view-options {
            padding: 6px 12px
        }
    ...
    <div class="main-container">
        ...
        <div class="header-section">
            ...
            <div class="header-right">
                ...
                <button class="btn btn-icon"
                    onclick="exportDataTable('inventory-table', '{{ __('Inventory Valuation Summary') }}', 'print')"><i
                        class="fa fa-print"></i></button>
                <button class="btn btn-icon" title="Export" id="btn-export"><i class="fa fa-external-link-alt"></i></button>
                <a class="btn-icon" title="{{ __('Import') }}" href="#"
                    data-url="{{ route('productservice.file.import') }}" data-ajax-popup="true"><i
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content p-0">
                    ...
                    <div class="modal-body text-center row">
                        <div class="col-md-6">
                            <button onclick="exportDataTable('inventory-table', '{{ __('Inventory Valuation Summary') }}')"
                        ...
                        <div class="col-md-6">
                            <button
                                onclick="exportDataTable('inventory-table', '{{ __('Inventory Valuation Summary') }}', 'pdf')"
                                class="btn btn-success mx-auto w-75 justify-content-center text-center"
        ...
        <div class="filter-section">
            ...
            <div class="options-row mt-2">
                <a href="#" class="columns-btn" id="columns-btn"><i class="fas fa-columns"></i>
                    {{ __('Columns') }}</a>
                <a href="#" class="filter-btn" id="filter-btn"><i class="fas fa-filter"></i> {{ __('Filter') }}</a>
                <a href="#" class="general-options" id="general-options-btn"><i class="fa fa-cog"></i>
                    {{ __('General options') }}</a>
            </div>
        </div>
    ...
    <div class="modal-overlay" id="general-options-overlay">
        <div class="drawer general-options-modal">
            <div class="modal-header">
                <h5>{{ __('General options') }} <i class="fa fa-info-circle"
                        title="{{ __('Configure report settings') }}"></i></h5>
            ...
            <div class="modal-content">
                ...
                <div class="option-section">
                    ...
                    <div class="option-group">
                        <label class="checkbox-label"><input type="checkbox" id="divide-by-1000">
                            {{ __('Divide by 1000') }}</label>
                ...
                <div class="option-section">
                    ...
                    <div class="option-group">
                        <div class="negative-format-group">
                            ...
                            <label class="checkbox-label"><input type="checkbox" id="show-in-red">
                                {{ __('Show in red') }}</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="option-section">
                    ...
                    <div class="option-group">
                        <label class="checkbox-label"><input type="checkbox" id="company-logo">
    ...
    <div class="modal-overlay" id="view-options-overlay">
        <div class="drawer">
            <div class="modal-header">
                <h5>{{ __('View options') }} <i class="fa fa-info-circle"
                        title="{{ __('Adjust how the report looks') }}"></i></h5>
            ...
            <div class="modal-content">
                ...
                <div class="option-section">
                    ...
                    <div class="option-group">
                        <label class="checkbox-label"><input type="checkbox" id="opt-compact">
                            {{ __('Compact rows') }}</label>
                    </div>
                </div>
                ...
                <div class="option-section">
                    ...
                    <div class="option-group">
                        ...
                        <label class="checkbox-label"><input type="checkbox" id="opt-borders">
                            {{ __('Show borders') }}</label>
                        <label class="checkbox-label"><input type="checkbox" id="opt-wrap">
                            {{ __('Wrap long text') }}</label>
    ...
    <script>
        $(function() {
            ...
            $('#report-period').on('change', function() {
                ...
                switch ($(this).val()) {
                    ...
                    case 'this_year_to_date':
                        ...
                        break;
                    case 'last_month':
                        s = now.clone().subtract(1, 'month').startOf('month');
                        e = now.clone().subtract(1, 'month').endOf('month');
                        break;
                    case 'last_year':
                        s = now.clone().subtract(1, 'year').startOf('year');
                        e = now.clone().subtract(1, 'year').endOf('year');
                        break;
                    case 'all_dates':
                        s = moment('1900-01-01');
                        e = now.clone();
            ...
            $('#close-filter, #filter-overlay').on('click', function(e) {
                if (e.target.id === 'filter-overlay' || e.target.id === 'close-filter') $('#filter-overlay')
                    .hide();
            });
            ...
            function getDT(cb) {
                const tryGet = function(n) {
                    const dt = $.fn.dataTable.isDataTable('#inventory-table') ? $('#inventory-table')
                        .DataTable() : null;
                    if (dt) cb(dt);
                    else if (n > 0) setTimeout(() => tryGet(n - 1), 100);
                };
                tryGet(30);
            }
            $('#columns-btn').on('click', e => {
                e.preventDefault();
            ...
            $('.columns-list input[type="checkbox"]').on('change', function() {
                ...
                getDT(function(dt) {
                    const currentIndex = dt.colReorder && typeof dt.colReorder.transpose ===
                        'function' ?
            ...
            function applyHeaderFooterSettings(options) {
                ...
                if (!$('.report-footer').length) {
                    $('.report-content').append(
                        '<div class="report-footer" style="padding:12px 20px;border-top:1px solid #e6e6e6;font-size:12px;color:#6b7280;text-align:' +
                        options.footerAlignment + ';"></div>');
                }
                const now = new Date();
                const parts = [];
            ...
            $('#close-view-options, #view-options-overlay').on('click', function(e) {
                if (e.target.id === 'view-options-overlay' || e.target.id === 'close-view-options') $(
            ...
            $('#inventory-table').on('draw.dt', function() {
                $('#inventory-table tbody tr').each(function() {
                    $(this).find('td').each(function() {
                        ...
                        if (!txt) return;
                        if (/^-/.test(txt) || /\((.*?)\)/.test(txt) || /-$/.test(txt)) $(
                            this).addClass('negative-amount');
                        if (txt === '0' || txt === '0.0' || txt === '0.00' || txt ===
                            '{{ \Auth::user()->priceFormat(0) }}') $(this).addClass(
                            'zero-amount');
                    });
                });
            });
            ...
            function resizeInventoryDT() {
                ...
                $(table.table().container()).find('.dataTables_scrollBody').css({
            ...
            $(window).on('resize', function() {
                ...
                _rsTimer = setTimeout(resizeInventoryDT, 120);
            });
        });

        function refreshData() {
            $('#inventory-table').DataTable().ajax.reload(null, false);
        }
    </script>
@endpush
