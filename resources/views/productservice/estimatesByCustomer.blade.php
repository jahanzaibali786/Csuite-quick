...
@section('content')
    ...
    <style>
        ...
        .btn-success{background:#22c55e;color:#fff;font-weight:500}.btn-success:hover{background:#16a34a}
        ...
        .right-controls{display:flex;align-items:center;gap:8px;margin-left:auto;flex-shrink:0;white-space:nowrap}
        .btn-qb-option,.btn-qb-action{background:transparent;border:none;padding:6px 10px;font-size:13px;font-weight:500;display:inline-flex;align-items:center;border-radius:4px;white-space:nowrap;cursor:pointer;transition:.15s}
        .btn-qb-option{color:#0066cc}.btn-qb-option:hover{background:#f0f7ff;color:#0052a3}
        .btn-qb-action{color:#6b7280}.btn-qb-action:hover{background:#f3f4f6;color:#374151}
        .btn-qb-option i,.btn-qb-action i{margin-right:4px;font-size:12px}
        ...
        .general-options-modal,.columns-modal,.view-options-modal{background:#fff;position:fixed;top:0;right:0;bottom:0;height:100%;width:360px;max-width:90vw;margin:0;border-radius:0;box-shadow:-2px 0 10px rgba(0,0,0,.1);overflow-y:auto;animation:slideInRight .18s ease-out}
        ...
        .qb-col-item{display:flex;align-items:center;gap:10px;padding:10px 6px;border-radius:6px;border-bottom:1px solid #f3f4f6}
        ...
        .qb-ghost{opacity:.6;background:#eef2ff}.qb-chosen{background:#f1f5f9}

        /* Print */
        @media print{.controls-bar,.report-header{display:none!important}.quickbooks-report{background:#fff!important}.report-content{box-shadow:none!important;margin:0!important}.product-service-table{font-size:11px}.product-service-table th,.product-service-table td{padding:6px 4px}}
...
@push('script-page')
    ...
    <script>
        $(function(){
            ...
            function markNow(){ lastUpdatedAt=Date.now(); renderLast(); if(tickerId) clearInterval(tickerId); tickerId=setInterval(renderLast, 30000); }
            markNow();
            ...
            window.reportOptions = {
                ...
                companyLogo:false, companyName:true, reportPeriod:true, headerAlignment:'center',
            ...
            function withDT(cb){
                const dt = window.LaravelDataTables ? window.LaravelDataTables['proposals-by-customer-table'] : null;
                if (dt) return cb(dt);
            ...
            withDT(function(dt){
                try { new $.fn.dataTable.FixedHeader(dt); } catch(e){}
                try { new $.fn.dataTable.ColReorder(dt); } catch(e){}
                    ...
                    new Sortable(document.getElementById('qb-columns-list'), {
                        ...
                        onEnd: function(){
                            ...
                            if (dt.colReorder && typeof dt.colReorder.order === 'function') {
                                try { dt.colReorder.order(newOrder, true); dt.columns.adjust().draw(false); } catch(e){}
                            }
                        }
                    });
                }
                ...
                // Refresh stamp
                $('#proposals-by-customer-table').on('xhr.dt', function(){ markNow(); $('#btn-refresh i').removeClass('fa-spin'); });
            });

            function refreshData(){ withDT(dt => dt.draw(false)); }

            // Filter UI -> refresh
            $('#filter-customer-modal,#filter-status-modal,#filter-start-date-modal,#filter-end-date-modal').on('change', function(){
                updateDateRangeDisplay(); refreshData();
            });

            function updateDateRangeDisplay(){
                const s=$('#filter-start-date-modal').val(), e=$('#filter-end-date-modal').val();
                if (s && e) $('#date-range-display').text(new Date(s).toLocaleDateString() + ' - ' + new Date(e).toLocaleDateString());
            ...
            $('#columns-btn').on('click',()=>{$('#columns-overlay').show();});
            $('#filter-btn').on('click',()=>$('#filter-overlay').show());
            $('#general-options-btn').on('click',()=>$('#general-options-overlay').show());

            $('#close-view-options').on('click',()=>{$('#view-options-overlay').hide(); applyViewOptions();});
            $('#close-columns').on('click',()=>$('#columns-overlay').hide());
            $('#close-filter').on('click',()=>{$('#filter-overlay').hide(); refreshData();});
            $('#close-general-options').on('click',()=>{$('#general-options-overlay').hide(); applyGeneralOptionsFromUI(); refreshData();});

            $('.modal-overlay').on('click', function(e){ if (e.target === this){ $(this).hide(); }});
            $(document).on('keydown', e => { if (e.key==='Escape') $('.modal-overlay').hide(); });
            ...
            /* ============ Header actions ============ */
            $('#btn-refresh').on('click', function(){ $(this).find('i').addClass('fa-spin'); refreshData(); });
            ...
            $('#btn-save').on('click', function(){
                const name = prompt('Enter report name:', '{{ $pageTitle }} - ' + new Date().toISOString().slice(0,10));
