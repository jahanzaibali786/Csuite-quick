...
@section('content')
    <style>
        ...
        .quickbooks-report{
            ...
            background:#f5f6fa;min-height:100vh;color:#262626;
        }
        ...
        .btn-outline{background:#fff;color:#374151;padding:8px 12px;font-size:13px;}
        ...
        .btn-quickbooks{background:#ffffff;border:1px solid #d1d5db;border-radius:4px;color:#374151;padding:6px 12px;font-size:13px;font-weight:500;cursor:pointer;display:inline-flex;align-items:center;text-decoration:none;transition:all 0.15s ease;}
        .btn-quickbooks:hover{background:#f9fafb;border-color:#9ca3af;color:#1f2937;}
        .btn-quickbooks:active{background:#f3f4f6;border-color:#6b7280;}
        .btn-quickbooks i{margin-right:4px;font-size:12px;}
        ...
        /* Table */
        .table-container{background:#fff;max-height:500px;overflow-y:auto;} /* Add scrolling container */
        ...
        .product-service-table td{padding:12px 16px;border-bottom:1px solid #f3f4f6;color:#262626;}
        .product-service-table tbody tr:hover{background:#f9fafb;}
        .product-service-table .text-right{text-align:right;}
        .product-service-table tfoot th{background:#f8f9fa;border-top:2px solid #dee2e6;font-weight:bold;position:sticky;bottom:0;z-index:9;}
        .product-service-table .total-row{background:#f8f9fa !important;}
        ...
        .filter-group select{width:100%;padding:8px 12px;border:1px solid #ddd;border-radius:4px;font-size:13px;background:#fff;color:#262626;height:36px;}
  ...
  .controls-bar { padding: 10px 24px; }
  .controls-inner{
    display:flex;
  ...
  .left-controls{
    display:flex;
    align-items:center !important;  /* override earlier flex-end */
    gap:12px;
    flex-wrap:nowrap;                /* single row */
    min-width:0;
  }
  ...
  .filter-label{ margin:0; line-height:1; font-size:12px; white-space:nowrap; }

  /* Normalize heights for selects/inputs/buttons in this row */
  .controls-bar .form-control,
  .controls-bar .form-select{ height:32px; padding:6px 8px; font-size:13px; }
  .controls-bar .btn-qb-option,
  .controls-bar .btn-qb-action,
  .controls-bar .btn-quickbooks{
    height:32px; display:inline-flex; align-items:center; line-height:30px;
  }

  /* Keep the ‘View options’ link visually aligned with fields */
  #view-options-btn{ padding:6px 10px; align-self:center; }
                ...
                <table class="table product-service-table" style="margin-bottom:0;border-top:2px solid #dee2e6;">
                    <tbody>
                        <tr class="total-row" style="background-color: #f8f9fa; font-weight: bold;">
                            ...
                            <td class="text-right" style="font-weight: bold; padding: 12px 16px; border-bottom: none;" id="total-income-display">$ 0.00</td>
                            <td class="text-right" style="font-weight: bold; padding: 12px 16px; border-bottom: none;" id="total-expenses-display">$ 0.00</td>
                            <td class="text-right" style="font-weight: bold; padding: 12px 16px; border-bottom: none;" id="total-net-display">$ 0.00</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <div class="filter-group">
                    ...
                    <select id="filter-customer-name" class="form-control">
                        ...
                        @foreach($customers as $customerName)
                            <option value="{{ $customerName }}" {{ ($filter['selectedCustomerName'] ?? '') == $customerName ? 'selected' : '' }}>
                                {{ $customerName }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        $(function () {
            ...
            $('#btn-save').on('click', function(){
                const name = prompt('Enter report name:', 'Income By Customer Summary - ' + new Date().toISOString().slice(0,10));
            ...
            function applyViewOptions(){
                ...
                if ($('#opt-equal-width').prop('checked')) css += '.product-service-table th,.product-service-table td{width:25%;}';
                ...
                if ($('#opt-sticky-head').prop('checked')){
                    ...
                    $('.product-service-table thead th').css({'position':'sticky','top':'0','z-index':'10','background':'#f9fafb'});
                    $('.product-service-table tfoot th').css({'position':'sticky','bottom':'0','z-index':'9','background':'#f8f9fa'});
                }else{
                    $('.product-service-table thead th').css({'position':'static'});
                    $('.product-service-table tfoot th').css({'position':'static'});
                }
            }
