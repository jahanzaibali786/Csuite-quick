...
<style>
    ...
    /* Controls row */
    .controls-bar{ background:#fff; padding:10px 24px; border-bottom:1px solid #e6e6e6; }
    .controls-inner{ display:flex; justify-content:space-between; align-items:center; gap:14px; flex-wrap:nowrap; }
    .left-controls{ display:flex; align-items:center!important; gap:12px; flex-wrap:nowrap; min-width:0; }
    .right-controls{ display:flex; gap:8px; align-items:center; margin-left:auto; }
    ...
    .btn-qb-option{ background:transparent; border:none; color:#0066cc; padding:6px 10px; font-size:13px; font-weight:500; cursor:pointer; display:inline-flex; align-items:center; transition:.15s; white-space:nowrap; }
    .btn-qb-option:hover{ background:#f0f7ff; color:#0052a3; }
    .btn-qb-option i{ margin-right:4px; font-size:12px; }

    .btn-qb-action{ background:transparent; border:none; color:#6b7280; padding:6px 10px; font-size:13px; font-weight:500; cursor:pointer; display:inline-flex; align-items:center; border-radius:4px; white-space:nowrap; transition:.15s; }
    ...
    .deposit-table th{ background:#f9fafb; border-bottom:2px solid #e5e7eb; padding:12px 16px; text-align:left; font-weight:600; color:#374151; font-size:12px; text-transform:uppercase; letter-spacing:.025em; position:sticky; top:0; z-index:10; }
    .deposit-table td{ padding:12px 16px; border-bottom:1px solid #f3f4f6; color:#262626; vertical-align:middle; }
    .deposit-table tbody tr:hover{ background:#f9fafb; }
    ...
    @media print{
        ...
        .deposit-table th,.deposit-table td{ padding:6px 4px; }
    }
</style>
            ...
            <div class="filter-group">
                ...
                <select id="filter-vendor-name" class="form-control">
                    ...
                    @foreach($vendors as $vendorName)
                        <option value="{{ $vendorName }}" {{ (request('vendor_name') ?? '') == $vendorName ? 'selected' : '' }}>{{ $vendorName }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
$(function(){
    ...
    function dt(){ return window.LaravelDataTables && window.LaravelDataTables[TABLE_ID]; }

    /* ===== Last updated ticker ===== */
    const $last = $('.last-updated');
    let lastUpdatedAt = Date.now(), tickerId = null;
    function rel(ts){ const s=Math.floor((Date.now()-ts)/1000);
    ...
    function formatAmount(raw, isMoney=true){
        ...
        const isNeg=val<0, negFmt=o.negativeFormat||'-100';
        let core=absText;
        if(isNeg){ if(negFmt==='(100)') core=`(${absText})`; else if(negFmt==='100-') core=`${absText}-`; else core=`-${absText}`; }
        let html=core;
        if(isMoney){
    ...
    function applyNumericReformat(){
        const $table = $(`#${TABLE_ID}`); if(!$table.length) return;
        ...
        $table.find('tbody tr').each(function(){
            $(this).children('td').each(function(i){
                const h=headerTexts[i]||''; const isMoney=moneyHeaders.some(k=>h.includes(k)); const isQty=qtyHeaders.some(k=>h.includes(k));
                if(!isMoney && !isQty) return;
                const raw=$(this).text(); const out=formatAmount(raw, isMoney);
                $(this).html(`<span class="${out.classes}">${out.html}</span>`).addClass('text-right');
            });
        });
    }
    function applyGeneralOptions(triggerRedraw=true){
        ...
        numberCSS(o); headerApply(o); footerRender(o); applyNumericReformat();
    ...
    function applyViewOptions(){
        ...
        css += $('#opt-compact').prop('checked') ? '.deposit-table th,.deposit-table td{padding:8px 12px;}' : '.deposit-table th,.deposit-table td{padding:12px 16px;}';
        css += $('#opt-hover').prop('checked') ? '.deposit-table tbody tr:hover{background:#f9fafb;}' : '.deposit-table tbody tr:hover{background:inherit;}';
        if($('#opt-striped').prop('checked')) css += '.deposit-table tbody tr:nth-child(even){background-color:#f8f9fa;}';
        css += $('#opt-borders').prop('checked') ? '.deposit-table th,.deposit-table td{border:1px solid #e5e7eb;}' : '.deposit-table th,.deposit-table td{border:none;border-bottom:1px solid #f3f4f6;}';
        css += $('#opt-wrap').prop('checked') ? '.deposit-table th,.deposit-table td{white-space:normal;word-wrap:break-word;}' : '.deposit-table th,.deposit-table td{white-space:nowrap;}';
        css += $('#opt-auto-width').prop('checked') ? '.deposit-table{table-layout:auto;}' : '.deposit-table{table-layout:fixed;}';
    ...
    function pushUrlAndReload(){
        ...
        const cust=$('#filter-customer-name').val(); const vend=$('#filter-vendor-name').val();
        ...
        if(vend) url.searchParams.set('vendor_name',vend); else url.searchParams.delete('vendor_name');
    ...
    function handlePeriodChange(){
        ...
        let startDate='', endDate=''; const dcopy=d=>new Date(d.getTime());
        switch(period){
    ...
    $('#start-date, #end-date, #accounting-method, #filter-customer-name, #filter-vendor-name').on('change', function(){ updateHeaderDate(); pushUrlAndReload(); });

    /* ===== Columns Drawer ===== */
    function transposeIndex(orig){
        const api=dt();
        return (api && api.colReorder && typeof api.colReorder.transpose==='function') ? api.colReorder.transpose(orig, 'toCurrent') : orig;
    }
            ...
            new Sortable(document.getElementById('qb-columns-list'), {
                ...
                onEnd:function(){
                    ...
                    if(api && api.colReorder && typeof api.colReorder.order==='function'){
                        try{ api.colReorder.order(newOrder, true); localStorage.setItem('deposit-detail-col-order', JSON.stringify(newOrder)); api.columns.adjust().draw(false);}catch(e){}
                    }
                }
            });
        }
    ...
    $('#btn-save').on('click', function(){
        const name = prompt('Enter report name:', 'Deposit Detail - ' + new Date().toISOString().slice(0,10));
        if(name) alert('Report "'+name+'" would be saved with current settings.');
    });

    /* ===== Bind drawers/live changes ===== */
    $('#apply-general-options').on('click', function(){ applyGeneralOptions(true); $('#general-options-overlay').removeClass('drawer-open'); });
    ...
    $(document).on('xhr.dt', `#${TABLE_ID}`, function(){
        ...
        applyGeneralOptions(false); applyViewOptions(); footerRender(window.reportOptions);
    });
    $(document).on('draw.dt', `#${TABLE_ID}`, function(){ applyNumericReformat(); });

    /* ===== Initial paint ===== */
    setTimeout(function(){ applyGeneralOptions(false); applyViewOptions(); footerRender(window.reportOptions); }, 120);
});
</script>
