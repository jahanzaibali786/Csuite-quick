...
@section('page-title')
    {{ __('Sales by Customer Detail') }}
@endsection

@push('script-page')
    <!-- jQuery (if not already included by your layout footer) -->
    <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>

    <!-- DataTables core + Bootstrap skin (optional) -->
    ...
    <!-- (Optional) Moment for date text in header footer; not required for server filters -->
...
<style>
    ...
    .controls-inner { display: flex; justify-content: space-between; align-items: center; gap: 14px; flex-wrap: nowrap; max-width: 100%; }
    .left-controls { display: flex; align-items: center !important; gap: 12px; flex-wrap: nowrap; flex-shrink: 1; min-width: 0; }
    .right-controls { display: flex; gap: 8px; align-items: center; flex-shrink: 0; margin-left: auto; }
    ...
    .btn-qb-option { background: transparent; border: none; color: #0066cc; padding: 6px 10px; font-size: 13px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; text-decoration: none; transition: all 0.15s ease; white-space: nowrap; }
    .btn-qb-option:hover { background: #f0f7ff; color: #0052a3; }
    .btn-qb-option i { margin-right: 4px; font-size: 12px; }

    .btn-qb-action { background: transparent; border: none; color: #6b7280; padding: 6px 10px; font-size: 13px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; text-decoration: none; transition: all 0.15s ease; border-radius: 4px; white-space: nowrap; }
    ...
    /* Table */
    .table-container { background: #fff; max-height: 520px; overflow: auto; }
    .product-service-table { width: 100%; border-collapse: collapse; font-size: 13px; }
    .product-service-table th { background: #f9fafb; border-bottom: 2px solid #e5e7eb; padding: 12px 16px; text-align: left; font-weight: 600; color: #374151; font-size: 12px; text-transform: uppercase; letter-spacing: .025em; position: sticky; top: 0; z-index: 10; }
    .product-service-table td { padding: 12px 16px; border-bottom: 1px solid #f3f4f6; color: #262626; vertical-align: middle; }
...
<div class="quickbooks-report">
    ...
    <div class="report-header">
        <h4 class="mb-0">{{ __('Sales by Customer Detail') }}</h4>
    ...
    <div class="controls-bar">
        <div class="controls-inner">
            <div class="left-controls">
                ...
                <button class="btn btn-qb-option mt-4" id="view-options-btn">
    ...
    <div class="report-content">
        <div class="report-title-section">
            <h2 class="report-title">{{ __('Sales by Customer Detail') }}</h2>
        ...
        <div class="table-container">
            {!! $dataTable->table(['class' => 'table product-service-table', 'id' => 'sales-by-customer-detail-table']) !!}
        </div>
    </div>
</div>
            ...
            <div class="filter-group">
                ...
                <select id="filter-customer-name" class="form-control">
                    ...
                    @foreach($customers as $customerName)
                        <option value="{{ $customerName }}" {{ (request('customer_name') ?? '') == $customerName ? 'selected' : '' }}>{{ $customerName }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
<script>
/* ========= CONFIG ========= */
const TABLE_ID = 'sales-by-customer-detail-table'; // <-- keep your Yajra table id
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderLast(){ if ($last.length) $last.text(`Last updated ${rel(lastUpdatedAt)}`); }
  ...
  window.reportOptions = {
    ...
    negativeFormat:'-100', // '-100' | '(100)' | '100-'
    showInRed:false,
  ...
  /* ========= Apply General Options (supports BOTH sets of controls) ========= */
  function readGeneralOptionsFromUI() {
    const o = window.reportOptions;

    // Numbers
    o.divideBy1000 = $('#divide-by-1000').prop('checked') || false;

    // allow legacy id from your file: #without-cents -> roundWholeNumbers
    const withoutCents = $('#without-cents').prop('checked');
    const roundWhole = $('#round-whole-numbers').prop('checked');
    o.roundWholeNumbers = (typeof roundWhole === 'boolean') ? roundWhole : !!withoutCents;

    // show/hide zeros (IBCS: hide-zero-amounts) vs your file: show-zero-amounts
    const hideZeroIBCS = $('#hide-zero-amounts').prop('checked');
    const showZeroLegacy = $('#show-zero-amounts').prop('checked');
    // precedence: if legacy exists, hideZero = !showZeroLegacy; else use IBCS
    o.hideZeroAmounts = (typeof showZeroLegacy === 'boolean') ? !showZeroLegacy : !!hideZeroIBCS;

    // negative format: either a select#negative-format OR radio[name=negative]
    if ($('#negative-format').length) {
      o.negativeFormat = $('#negative-format').val() || '-100';
    } else {
      // radios: minus | parentheses
      const neg = $('input[name="negative"]:checked').val();
      o.negativeFormat = (neg === 'parentheses') ? '(100)' : '-100';
    }

    // red negatives (only in IBCS UI)
    o.showInRed = $('#show-in-red').prop('checked') || false;

    // Header
    o.companyLogo  = $('#company-logo').prop('checked') || false;
    o.reportTitle  = $('#opt-report-title').prop('checked') !== false; // default true
  ...
  function applyViewOptions(){
    ...
    if ($('#opt-equal-width').prop('checked')) css += '.product-service-table th,.product-service-table td{width:10%;}';
    const fs = $('#font-size').val() || '13px';
    css += `.product-service-table, .product-service-table th, .product-service-table td{font-size:${fs};}`;
    css += '</style>';
    $('head').append(css);
  }

  /* ========= Numeric reformat pass on currently-rendered cells ========= */
  function applyNumericReformat(){
    const $table = $(`#${TABLE_ID}`);
    if (!$table.length) return;

    // detect columns by header text
    const moneyHeaders = ['amount','balance','price','total','tax'];
    const qtyHeaders   = ['qty','quantity'];

    const headerTexts = [];
    $table.find('thead th').each(function(i){ headerTexts[i] = ($(this).text() || '').trim().toLowerCase(); });
    ...
    $table.find('tbody tr').each(function(){
      $(this).children('td').each(function(i){
        ...
        $(this).html(`<span class="${out.classes}">${out.html}</span>`);
        $(this).addClass('text-right');
      });
    });
  }

  /* ========= Bind General Options controls ========= */
  // If you have an Apply button:
  ...
  $('.general-options-modal').on('change', 'input,select', function(){ applyGeneralOptions(false); });

  // Expand/collapse sections chevron
  ...
  $('#columns-btn').on('click', () => { buildColumnsList(); $('#columns-overlay').addClass('drawer-open'); });
  ...
  $('#close-general-options').on('click', () => $('#general-options-overlay').removeClass('drawer-open'));
  ...
  /* ========= Period + Filters (unchanged from your flow) ========= */
  ...
  function pushUrlAndReload(){
    ...
    const start = $('#start-date').val(); const end = $('#end-date').val();
    const acct = $('#accounting-method').val(); const cust = $('#filter-customer-name').val();
    if (period && period !== 'all_dates') url.searchParams.set('report_period', period); else url.searchParams.delete('report_period');
    ...
    if (acct && acct !== 'accrual') url.searchParams.set('accounting_method', acct); else url.searchParams.delete('accounting_method');
    if (cust) url.searchParams.set('customer_name', cust); else url.searchParams.delete('customer_name');
    window.history.replaceState({}, '', url);
    if (dt()) dt().draw();
    markNow();
  }
  function handlePeriodChange(){
    const period = $('#report-period').val(); const today = new Date();
    ...
    switch(period){
      ...
      case 'all_dates': startDate=''; endDate=''; $('#date-range-display').text('All Dates'); pushUrlAndReload(); return;
      case 'custom': default: return;
    }
    $('#start-date').val(startDate); $('#end-date').val(endDate);
    updateHeaderDate(); pushUrlAndReload();
  }
  $('#report-period').on('change', handlePeriodChange);
  $('#start-date, #end-date, #accounting-method, #filter-customer-name').on('change', function(){ updateHeaderDate(); pushUrlAndReload(); });

  /* ========= Columns drawer (optional, unchanged) ========= */
  function transposeIndex(orig){
    const api = dt();
    return (api && api.colReorder && typeof api.colReorder.transpose==='function')
      ? api.colReorder.transpose(orig, 'toCurrent') : orig;
  }
  function buildColumnsList(){
    const api = dt(); if (!api) return;
    const $list = $('#qb-columns-list'); $list.empty();
    const headers = api.columns().header().toArray();
    headers.forEach((h, origIdx) => {
      const curIdx = transposeIndex(origIdx), visible = api.column(curIdx).visible();
      ...
      new Sortable(document.getElementById('qb-columns-list'), {
        ...
        onEnd: function(){
          ...
          if (api && api.colReorder && typeof api.colReorder.order === 'function') {
            try { api.colReorder.order(newOrder, true); localStorage.setItem('sbc-detail-col-order', JSON.stringify(newOrder)); api.columns.adjust().draw(false);} catch(e){}
          }
        }
      });
    }
    ...
    $list.off('change').on('change', 'input[type="checkbox"][data-col]', function(){
      ...
      try { api.column(curIndex).visible(visible, false); api.columns.adjust().draw(false);} catch(e){}
    });
  }

  /* ========= Header actions ========= */
  $('#btn-refresh').on('click', function(){ $(this).find('i').addClass('fa-spin'); if (dt()) dt().ajax.reload(); });
  ...
  $('#btn-save').on('click', function(){
    const name = prompt('Enter report name:', 'Sales by Customer Detail - ' + new Date().toISOString().slice(0,10));
    if (name) alert('Report "'+name+'" would be saved with current settings.');
  });

  /* ========= Re-apply on each DataTables lifecycle ========= */
  // When AJAX completes:
  ...
  // When table draws (paging / redraw):
