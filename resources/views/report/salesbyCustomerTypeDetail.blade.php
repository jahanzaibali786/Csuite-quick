...
@section('content')
    ...
    <style>
        ...
        /* Controls row (IBCS) */
        .controls-bar { background: #fff; padding: 8px 16px; border-bottom: 1px solid #e6e6e6; overflow: hidden; }
        ...
        #controls-row-fix .controls-bar { padding: 10px 24px; }
        #controls-row-fix .controls-inner { display: flex; align-items: center; gap: 14px; flex-wrap: nowrap; overflow: hidden; }
        #controls-row-fix .left-controls { display: flex; align-items: center !important; gap: 12px; flex-wrap: nowrap; min-width: 0; }
        #controls-row-fix .filter-item { display: flex; flex-direction: column; gap: 4px; margin: 0; }
        #controls-row-fix .filter-label { margin: 0; line-height: 1; font-size: 12px; white-space: nowrap; }
        #controls-row-fix .form-control, #controls-row-fix .form-select { height: 32px; padding: 6px 8px; font-size: 13px; }
        #controls-row-fix .btn-qb-option, #controls-row-fix .btn-qb-action { height: 32px; display: inline-flex; align-items: center; line-height: 30px; }
        #controls-row-fix #view-options-btn { padding: 6px 10px; align-self: center; }
        #controls-row-fix .right-controls { display: flex; align-items: center; gap: 8px; margin-left: auto; flex-shrink: 0; white-space: nowrap; }
        @media (max-width: 1020px){ #controls-row-fix .controls-bar { overflow-x: auto; } #controls-row-fix .controls-inner { min-width: max-content; } }
        ...
        .product-service-table .text-right { text-align: right; }
        ...
        $(function () {
            ...
            const table = $('#ledger-table').DataTable({
                ...
                columns: [
                    ...
                    { data:'memo_description', name:'memo_description' },
                    ...
                    { data:'sales_with_tax', name:'sales_with_tax', className:'text-right',
                      render: function(data, type, row){
                        if (type !== 'display') return data;
            ...
            $('#btn-save').on('click', function(){
                const name = prompt('Enter report name:', 'Sales by Customer Type Detail - ' + new Date().toISOString().slice(0,10));
            ...
            $('#start-date, #end-date, #accounting-method').on('change', function(){
                ...
                const url = new URL(window.location.origin + window.location.pathname);
                if (reportPeriod && reportPeriod !== 'all_dates') url.searchParams.set('report_period', reportPeriod);
                if (startDate) url.searchParams.set('start_date', startDate);
                if (endDate) url.searchParams.set('end_date', endDate);
                if (accountingMethod && accountingMethod !== 'accrual') url.searchParams.set('accounting_method', accountingMethod);
                table.ajax.url(url.href).load(null, false);
                markNow();
            });
                    ...
                    onEnd: function(){
                        ...
                        if (table && table.colReorder && typeof table.colReorder.order === 'function') {
                            try {
                                ...
                                localStorage.setItem('sales-by-customer-type-column-order', JSON.stringify(newOrder));
