...
@push('script-page')
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateTableHeaders(endDate) {
                const currentDate = moment(endDate, 'YYYY-MM-DD').format('YYYY-MM-DD');
                const previousDate = moment(endDate, 'YYYY-MM-DD').subtract(1, 'year').format('YYYY-MM-DD');

                const table = $('#aging-summary-table').DataTable();
                table.column(1).header().innerHTML = 'As of ' + currentDate;   // Current period
                table.column(2).header().innerHTML = 'As of ' + previousDate;  // Previous year
            }
            ...
            $('#filter-start-date, #filter-end-date, #filter-account, #accounting-method').on('change', function() {
                const endDate = $('#filter-end-date').val();
                updateTableHeaders(endDate);            // Update headers dynamically
                $('#aging-summary-table').DataTable().ajax.reload(); // Reload table data
            });
